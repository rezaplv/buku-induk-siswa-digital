...
const PDFGenerator = {
  ...
  buildEkskulRows(kegiatan, keterangan, semester, minRows) {
    ...
    const rowCount = Math.max(kegiatanList.length, keteranganList.length, 1);
    let rows = '';
    for (let i = 0; i < rowCount; i++) {
      const keg = esc(kegiatanList[i] || '') || '&nbsp;';
      const ket = esc(keteranganList[i] || '') || '&nbsp;';
      rows += '<tr>';
      if (i === 0) {
        rows += '<td class="center" style="vertical-align:middle;" rowspan="' + rowCount + '">' + semester + '</td>';
      }
      rows += '<td style="vertical-align:top;">' + keg + '</td>' +
