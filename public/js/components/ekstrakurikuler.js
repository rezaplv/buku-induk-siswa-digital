...
const Ekstrakurikuler = {
  ...
  async saveAll() {
    ...
    for (let i = 0; i < this.studentList.length; i++) {
      ...
      if (existing) {
        // Update only the relevant semester fields without overwriting other data
        const updated = { ...existing };
        updated[kegField] = this.kegiatanValues[i] || '';
        updated[ketField] = this.keteranganValues[i] || '';
        await DB.updateNonAkademik(updated);
      } else {
        // Create new record with only the current semester data
        const newRecord = {
  ...
  render() {
    ...
    page.innerHTML = `
      <div class="page-header"><h2 class="page-title">Ekstrakurikuler</h2></div>
      <div class="card" style="margin-bottom:16px;">
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-end;">
          <div class="form-group" style="margin-bottom:0;min-width:180px;">
            <label>Tahun Pelajaran</label>
            <select id="ekskul-tp" onchange="Ekstrakurikuler.onTPChange(this.value)">
          ...
          <div class="form-group" style="margin-bottom:0;min-width:180px;">
            <label>Kelas</label>
            <select id="ekskul-kelas" onchange="Ekstrakurikuler.onKelasChange(this.value)" ${!this.selectedTP ? 'disabled' : ''}>
          ...
          <div class="form-group" style="margin-bottom:0;min-width:150px;">
            ...
            <select id="ekskul-semester" onchange="Ekstrakurikuler.onSemesterChange(this.value)" ${!this.selectedKelas ? 'disabled' : ''}>
  ...
  renderInputArea() {
    ...
    return `
      <div class="card" style="margin-bottom:16px;">
        <h3 class="section-title">Paste Batch - ${escapeHTMLDash(semLabel)}</h3>
        <p class="muted" style="margin-bottom:12px;">Paste daftar kegiatan dan keterangan (satu baris per siswa, ${this.studentList.length} baris urut alfabet). Jika siswa punya lebih dari 1 ekskul, pisahkan dengan koma.</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div class="form-group" style="margin-bottom:0;">
            <label>Paste Kegiatan</label>
            <textarea id="paste-kegiatan-textarea" rows="6" style="width:100%;font-family:monospace;font-size:13px;padding:10px;border:1px solid var(--border-color);border-radius:8px;resize:vertical;" placeholder="Pramuka, Volly\nPMR\nPaskibra, Futsal\n..."></textarea>
          </div>
          <div class="form-group" style="margin-bottom:0;">
            <label>Paste Keterangan</label>
            <textarea id="paste-keterangan-textarea" rows="6" style="width:100%;font-family:monospace;font-size:13px;padding:10px;border:1px solid var(--border-color);border-radius:8px;resize:vertical;" placeholder="Mampu baris-berbaris, Teknik dasar voli\nTerampil P3K\nDisiplin gerak, Teknik passing\n..."></textarea>
          </div>
        </div>
        <button type="button" class="btn btn-outline" onclick="Ekstrakurikuler.applyPaste()" style="margin-top:12px;">Terapkan dari Paste</button>
      </div>
      <div class="card">
        <h3 class="section-title">Data Ekstrakurikuler - ${escapeHTMLDash(this.selectedKelas)} (${escapeHTMLDash(semLabel)})</h3>
        <p class="muted" style="margin-bottom:12px;">${this.studentList.length} siswa (urut alfabet). Bisa juga edit langsung di kolom tabel.</p>
